...
* Statements that have been designed to support scenarios in which mid-tier applications take user queries and send a modified version of them to APL ([application query statements](#application-query-statements)).
...
* A [pattern statement](./patternstatement.md), which can be used by applications that are
  built on top of APL and expose the query language to their users to inject themselves
...
* A [query parameters statement](./queryparametersstatement.md), which is used by applications
  that are built on top of APL to protect themselves against injection attacks (similar to
...
* A [restrict statement](./restrictstatement.md), which is used by applications that are built
  on top of APL to restrict queries to a specific subset of data in APL (including restricting
