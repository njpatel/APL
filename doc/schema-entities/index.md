...
APL queries execute in the context of some APL database that is attached
to a APL cluster. Data in the database is arranged in tables, which the query
...
* Clusters are entities that hold databases.
  ...
  For example, `cluster("https://help.apl.windows.net")` is a reference
  to a cluster that holds the `Samples` database.

* [Databases](./databases.md) are named entities that hold tables
  and stored functions. All APL queries run in the context of some database,
  ...
  `cluster("https://help.apl.windows.net").database("Samples")`
...
* [Tables](./tables.md) are named entities that hold data. A table has an ordered set
  ...
  `cluster("https://help.apl.windows.net").database("Samples").StormEvents` is
...
* [Stored functions](./stored-functions.md) are named entities that
  allow reuse of APL queries or query parts.

* [External tables](./externaltables.md) are entities that reference data stored outside APL database.
  External tables are used for exporting data from APL to external storage as well as for querying
  external data without ingesting it into APL.