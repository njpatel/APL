    ...
    public class QueryParser
    {
        ...
        private DataScopeClause ParseDataScopeClause()
        {
            if (PeekToken().Kind == SyntaxKind.DataScopeKeyword)
            {
                ...
                var value = ParseRequiredToken(APLFacts.DataScopeValues);
...
#region Query operator parameters
        ...
        {
            return !APLFacts.IsKeyword(name) // known keywords will still be one token
        ...
        private static HashSet<string> s_knownQueryOperaterParameterNames =
            new HashSet<string>(
                ...
                .Concat(APLFacts.KnownQueryOperatorParameterNames));
...
#region render
        ...
        private RenderOperator ParseRenderOperator()
        {
            ...
            if (keyword != null)
            {
                var chartType = ParseToken(APLFacts.ChartTypes) ?? ParseToken(SyntaxKind.IdentifierToken) ?? CreateMissingNameToken(APLFacts.ChartTypes);
