...
namespace APL.Language.Binding
{
    using Parsing;
    using Symbols;
    using Syntax;
...
#region query operators
            ...
            public override SemanticInfo VisitForkOperator(ForkOperator node)
            {
                ...
                try
                {
                    ...
                    for (int i = 0, n = node.Expressions.Count; i < n; i++)
                    {
                        ...
                        CheckQueryOperators(expr, APLFacts.ForkOperatorKinds, diagnostics);
            ...
            public override SemanticInfo VisitPartitionOperator(PartitionOperator node)
            {
                ...
                try
                {
                    ...
                    {
                        CheckQueryOperators(ps.Subquery, APLFacts.PostPipeOperatorKinds, diagnostics);
                    }
                    ...
                    if (node.WithClause != null)
                    {
                        ...
                        switch (node.WithClause)
                        {
                            case FacetWithOperatorClause c:
                                ...
                                CheckQueryOperators(c.Operator, APLFacts.ForkOperatorKinds, diagnostics);
            ...
            public override SemanticInfo VisitMvApplySubqueryExpression(MvApplySubqueryExpression node)
            {
                ...
                try
                {
                    CheckQueryOperators(node.Expression, APLFacts.PostPipeOperatorKinds, diagnostics, allowContextualRoot: true);
            ...
            public override SemanticInfo VisitRenderOperator(RenderOperator node)
            {
                ...
                try
                {
                    ...
                    _binder.CheckIsToken(node.ChartType, APLFacts.ChartTypes, true, diagnostics);
