...
namespace APL.Language
{
    ...
    /// <summary>
    /// The global state that a APL query is associated with.
    /// </summary>
    public sealed class GlobalState
    {
        ...
        /// <summary>
        /// The <see cref="APLCache"/> used to store additional accumulated global state.
        /// If caching is not enabled for this <see cref="GlobalState"/> this property will return null.
        /// </summary>
        public APLCache Cache { get; }
        ...
        private GlobalState With(
            ...
            Optional<APLCache> cache = default(Optional<APLCache>))
        {
        ...
        public GlobalState WithCache()
        {
            ...
            {
                return With(cache: new APLCache(this));
            }
        }
        ...
        {
            name = APLFacts.GetHostName(name) ?? name;

            if (this.Cluster != ClusterSymbol.Unknown
                && (APLFacts.IsClusterHostName(name, this.Cluster.Name)
                    || (APLFacts.IsClusterShortName(name, this.Cluster.Name) && APLFacts.IsAPLWindowsNet(this.Cluster.Name))))
            {
                return this.Cluster;
            }

            return this.Clusters.FirstOrDefault(c => APLFacts.IsClusterHostName(name, c.Name))
                ?? this.Clusters.FirstOrDefault(c => APLFacts.IsClusterShortName(name, c.Name) && APLFacts.IsAPLWindowsNet(c.Name))
                ?? this.Clusters.FirstOrDefault(c => APLFacts.IsClusterShortName(name, c.Name));
        }
