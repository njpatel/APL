...
namespace APL.Language.Editor
{
    ...
    public sealed class CodeScript
    {
        ...
        {
            return From(text, new APLCodeServiceFactory(globals));
        }

        /// <summary>
        /// The <see cref="GlobalState"/> used by APL <see cref="CodeBlock"/>s.
        /// </summary>
        public GlobalState Globals => this.Factory.GetFactory<APLCodeServiceFactory>()?.Globals;

        /// <summary>
        /// Creates a new <see cref="CodeScript"/> with the APL globals changed.
        /// </summary>
        public CodeScript WithGlobals(GlobalState newGlobals)
        {
            var APLFactory = this.Factory.GetFactory<APLCodeServiceFactory>();
            if (APLFactory != null)
            {
                return WithFactory(this.Factory.WithFactory(APLFactory.WithGlobals(newGlobals)));
            }
