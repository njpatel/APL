...
namespace APL.Language.Editor
{
    ...
    /// <summary>
    /// A <see cref="CodeService"/> for APL queries and commands.
    /// </summary>
    public class APLCodeService : CommonCodeService
    {
        private readonly string kind;
        private readonly GlobalState globals;

        private APLCode lazyUnboundCode;
        private APLCode lazyBoundCode;
        ...
        private APLCodeService(string text, GlobalState globals, APLCode code)
            : base(text)
        {
            if (globals == null)
                throw new ArgumentNullException(nameof(globals));

            this.kind = APLCode.GetKind(text);
            this.globals = globals;
            this.lazyBoundCode = code;
        }

        public APLCodeService(string text, GlobalState globals = null)
            : this(text, globals ?? GlobalState.Default, null)
        {
        }

        public APLCodeService(APLCode code)
        ...
        /// </summary>
        private static bool CanBeAnalyzed(APLCode code) => code.MaxDepth <= APLCode.MaxAnalyzableSyntaxDepth;

        /// <summary>
        /// Gets the <see cref="APLCode"/> for the text without waiting for semantic analysis.
        /// </summary>
        private bool TryGetBoundOrUnboundCode(CancellationToken cancellationToken, bool waitForAnalysis, out APLCode code)
        {
                ...
                && CanBeParsed(this.Text))
            {
                lock (this) // don't let multiple threads duplicate computation work
                {
                    try
                    {
                        ...
                        {
                            var newCode = APLCode.Parse(this.Text, this.globals);
        ...
        /// <summary>
        /// Gets the <see cref="APLCode"/> for the text with semantic analysis done.
        /// </summary>
        private bool TryGetBoundCode(CancellationToken cancellationToken, bool waitForAnalysis, out APLCode code)
        {
            if (this.lazyBoundCode == null 
                && this.codeException == null 
                && waitForAnalysis
                && CanBeParsed(this.Text))
            {
                lock (this) // don't let multiple threads duplicate computation work
                {
                    try
                    {
                        ...
                        {
                            var newCode = APLCode.ParseAndAnalyze(this.Text, this.globals, cancellationToken: cancellationToken);
                            Interlocked.CompareExchange(ref this.lazyBoundCode, newCode, null);
                        }
                    }
                    catch (Exception e)
            ...
            CancellationToken cancellationToken = default)
        {
                ...
                && CanBeAnalyzed(code))
            {
                ...
                foreach (var analyzer in APLAnalyzers.All)
                {
        ...
        public override IReadOnlyList<AnalyzerInfo> GetAnalyzers()
        {
            if (_analyzers == null)
            {
                _analyzers = APLAnalyzers.All.Select(a => new AnalyzerInfo(a.Name, a.Diagnostics)).ToReadOnly();
            }
        ...
        public static IReadOnlyList<Diagnostic> AnalyzerDiagnostics
        {
            get
            {
                ...
                {
                    _analyzerDiagnostics = new APLCodeService("").GetAnalyzers().SelectMany(a => a.Diagnostics).ToReadOnly();
                }
        ...
        public override ClassificationInfo GetClassifications(int start, int length, bool clipToRange = true, bool waitForAnalysis = true, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (this.TryGetBoundCode(cancellationToken, waitForAnalysis, out var code))
            {
                ...
                try
                {
                    ...
                    APLClassifier.GetClassifications(code.Syntax, start, length, clipToRange, classifications, cancellationToken);
        ...
        private static string GetOutlineCollapsedText(APLCode code)
        {
        ...
        public override bool ShouldAutoComplete(int position, char key, CancellationToken cancellationToken = default(CancellationToken))
        {
                ...
                && CanBeAnalyzed(code))
            {
                try
                {
                    return new APLCompleter(code, CompletionOptions.Default, cancellationToken)
        ...
        public override CompletionInfo GetCompletionItems(int position, CompletionOptions options = null, CancellationToken cancellationToken = default(CancellationToken))
        {
                ...
                && CanBeAnalyzed(code))
            {
                ...
                try
                {
                    return new APLCompleter(code, options ?? CompletionOptions.Default, cancellationToken)
        ...
        public override QuickInfo GetQuickInfo(int position, QuickInfoOptions options = null, CancellationToken cancellationToken = default(CancellationToken))
        {
                ...
                && CanBeAnalyzed(code))
            {
                ...
                try
                {
                    return new APLQuickInfoBuilder(this, code, options ?? QuickInfoOptions.Default).GetQuickInfo(position, cancellationToken);
                }
                catch (Exception)
                {
        ...
        public override RelatedInfo GetRelatedElements(int position, FindRelatedOptions options = FindRelatedOptions.None, CancellationToken cancellationToken = default(CancellationToken))
        {
                ...
                && CanBeAnalyzed(code))
            {
                try
                {
                    return new APLRelatedElementFinder(code).FindRelatedElements(position, options);
                }
                catch (Exception)
                {
                }
        ...
        public override FormattedText GetFormattedText(FormattingOptions options = null, int cursorPosition = 0, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (this.TryGetBoundOrUnboundCode(cancellationToken, true, out var code)
                && CanBeAnalyzed(code))
            {
                return APLFormatter.GetFormattedText(code.Syntax, options, cursorPosition);
            }
            else
            {
